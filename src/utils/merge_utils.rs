...
pub mod prelude {
    pub use super::merge_pdfs;
}

pub fn merge_pdfs(documents: &mut Vec<Document>) -> Result<(Document, Vec<usize>), Box<dyn std::error::Error>> {
    // Define a starting `max_id` (will be used as start index for object_ids).
    let mut max_id = 1;
    let mut pagenums = vec![1];
